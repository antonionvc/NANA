...
export async function participantsUpdate({ id, participants, action }) {
  ...
  switch (action) {
    ...
    case 'daradmin':
    case 'darpoder':
      text = (chat.sPromote || tradutor.texto3 || conn.spromote || '');
    case 'demote':
    case 'quitarpoder':
    case 'quitaradmin':
      if (!text) {
        text = (chat.sDemote || tradutor.texto4 || conn.sdemote || '');
      }
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;AntonioDJ 👑;;;\nFN:AntonioDJ 👑\nORG:AntonioDJ 👑\nTITLE:\nitem1.TEL;waid=5219992095479:+593 998 104 494\nitem1.X-ABLabel:AntonioDJ 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:AntonioDJ 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'AntonioDJ 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
